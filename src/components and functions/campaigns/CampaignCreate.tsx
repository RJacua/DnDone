...
import {
  ConfirmationPopUp,
  InformationPopUp,
} from "../assetsForDesign/DDoneConfirmation";
import {
  BanListTablePost,
  CharacterListTablePost,
  HomebrewListTablePost,
} from "../assetsForDesign/Tables";
...
// importa imagem fixa "nop"
import nopImg from "/src/assets/nop.png";

export default function CampaignCreate() {
  const { user } = useUser();
  const [characterList, setCharacterList] = useState<any[]>([]);
  const [filteredCharacterList, setFilteredCharacterList] = useState<any[]>([]);

  const fetchCharacters = async () => {
    try {
      const data = await getChars();
      const allFetchedItems = [...(data || [])];
  ...
  const handleCharactersChange = (updatedCharacters: any[]) => {
    setCharacters(updatedCharacters);
  };

  const [homebrewRules, setHomebrewRules] = useState<any[]>([]);
  const handleHomebrewRulesChange = (updatedHomebrewRules: any[]) => {
    setHomebrewRules(updatedHomebrewRules);
  };

  const [homebrewSpells, setHomebrewSpells] = useState<any[]>([]);
  const handleHomebrewSpellsChange = (updatedHomebrewSpells: any[]) => {
    setHomebrewSpells(updatedHomebrewSpells);
  };

  const [banClasses, setBanClasses] = useState<any[]>([]);
  const handleBanClassesChange = (updatedBan: any[]) => {
    setBanClasses(updatedBan);
  };

  const [banSpells, setBanSpells] = useState<any[]>([]);
  const handleBanSpellsChange = (updatedBan: any[]) => {
    setBanSpells(updatedBan);
  };

  const [banRaces, setBanRaces] = useState<any[]>([]);
  const handleBanRacesChange = (updatedBan: any[]) => {
  ...
  // usa glob para pegar o tema
  let themeUrl = themeImages["/src/assets/themes/nocampaign.png"];
  if (theme !== "") {
    const key = `/src/assets/themes/${theme.toLowerCase()}.png`;
    themeUrl =
      themeImages[key] || themeImages["/src/assets/themes/nocampaign.png"];
  }
    ...
    setFilteredCharacterList(
      characterList.filter(
        (el) =>
          ...
          !el.spells.some((spell) =>
            banSpellsIndex.includes(spell.index.toLowerCase())
          )
      )
    );
  };
  ...
  if (!user || !(user[0].role === "master")) {
    return (
      ...
      >
        ...
        >
          Please log in as Master to create a new campaign.
        </h1>
      </div>
    );
  }
